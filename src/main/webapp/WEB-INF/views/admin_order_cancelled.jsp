...
<c:if test="${empty aemail}">
	...
	<title>Rodrigos de Carnicos | Pedidos Cancelados </title>
...
<h3 class="text-center">Pedidos cancelados</h3>
	<table id="example" class="table table-hover table-bordered text-center">
          ...
          <br>${orders.product.name}</td>
						...
						<a href="${pageContext.request.contextPath}/admin/order/changeStatus/Delivered/${orders.id}" class="btn btn-success" onclick="return confirm('Cambiar el estado del pedido a Entregado?');">Entregar</a>
						 <hr> <a href="${pageContext.request.contextPath}/admin/order/changeStatus/Pending/${orders.id}" class="btn btn-warning" onclick="return confirm('Cambiar el estado del pedido a Pendiente?');">Pendiente</a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
