...
<c:if test="${empty aemail}">
	...
	<title>Rodrigos de Carnicos | Órdenes entregadas</title>
...
<h3 class="text-center">Órdenes Entregadas</h3>
	<table id="example" class="table table-hover table-bordered text-center">
          ...
          <br>${orders.product.name}</td>
							...
							<td><a href="${pageContext.request.contextPath}/admin/order/changeStatus/${orderStatus}/${orders.id}" class="btn btn-warning" onclick="return confirm('Cambiar el estado del pedido a Pendiente?');">Pendiente</a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
